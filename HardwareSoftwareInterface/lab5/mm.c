...
void* mm_malloc (size_t size) {
  ...
  // Updates the following block's previous used tag ,size
  ...
  removeFreeBlock(ptrFreeBlock);
	 ...
	 if((blockSize - reqSize) >  MIN_BLOCK_SIZE ){
		// update the length and later we update the flags
    ...
    //Updates the boundary tag only for free blocks
		*((size_t*)POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE))) = newBlock->sizeAndTags;
    // Add the new block to the free list and immediately coalesce newly
    // allocated memory space
     insertFreeBlock(newBlock);
     coalesceFreeBlock(newBlock);    
	} else {
		// Updates the following blocks preceding used
		// BlockInfo* nextBlock = (BlockInfo*)POINTER_ADD(ptrFreeBlock, (blockSize - WORD_SIZE));
   ...
   // update the flags for allocated block
...
void mm_free (void *ptr) {
  ...
  // mark the next block preceding flaf as unused
